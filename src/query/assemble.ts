...
- Card interaction accuracy: When recommending sideboard cards or removal against specific threats, verify the interaction actually works using Card Reference data. Check power/toughness against -X/-X effects, check mana values against conditional removal, check card types against targeted effects. For example, -5/-5 does NOT kill a 6/6. If you cannot verify an interaction from the Card Reference, say so rather than assuming.
- Numerical accuracy: when stating counts ("only one deck", "three pilots", etc.), count the items you are about to list BEFORE writing the sentence. The number you state must exactly match the number of items you then present.
